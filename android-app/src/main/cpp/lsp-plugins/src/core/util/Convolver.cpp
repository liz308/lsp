...
namespace lsp
{
    ...
    void Convolver::process(float *dst, const float *src, size_t count)
    {
        ...
        while (count > 0)
        {
            size_t sub_off      = nFrameOff & (CONVOLVER_MIN_DATA_BUF_SIZE - 1);
            ...
            if (sub_off == 0)
            {
                ...
                if (nBlocks > 0)
                {
                    ...
                    conv                = &vConvData[(nBlocksDone + 1) * fft_step];
                    float *xdst         = &vDataBuffer[nBlocksDone << (nRank - 1)];
            ...
            dsp::copy(&vFrame[nFrameOff], src, to_do);
