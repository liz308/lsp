...
namespace lsp
{
    ...
    void DynamicDelay::process(float *out, const float *in, const float *delay, const float *fgain, const float *fdelay, size_t samples)
    {
        ...
        {
            ssize_t shift   = lsp_limit(ssize_t(delay[i]), 0, nMaxDelay);
            ssize_t tail    = nHead - shift;
            if (tail < 0)
                tail           += nCapacity;
            size_t feed     = tail  + lsp_limit(fdelay[i], 0, shift);
            if (feed > nCapacity)
                feed           -= nCapacity;

            vDelay[nHead]   = in[i];
            float s         = vDelay[tail];
            vDelay[feed]   += s * fgain[i];
            out[i]          = vDelay[tail];
    ...
    void DynamicDelay::copy(DynamicDelay *s)
    {
        ...
        ssize_t dt      = nCapacity - count;
        ssize_t st      = s->nHead - count;
        if (st < 0)
            st         += s->nCapacity;
