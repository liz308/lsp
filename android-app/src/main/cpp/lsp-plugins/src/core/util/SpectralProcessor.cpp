...
namespace lsp
{
    ...
    void SpectralProcessor::update_settings()
    {
        ...
        dsp::fill_zero(pOutBuf, buf_size*4);
        ...
        while (count > 0)
        {
            ...
            if (nOffset >= frame_size)
            {
                if (pFunc != NULL)
                {
                    ...
                    dsp::pcomplex_r2c(pFftBuf, pInBuf, buf_size);
                    dsp::packed_direct_fft(pFftBuf, pFftBuf, nRank);
                    pFunc(pObject, pSubject, pFftBuf, nRank);
                    dsp::packed_reverse_fft(pFftBuf, pFftBuf, nRank);
                    dsp::pcomplex_c2r(pFftBuf, pFftBuf, buf_size);
                }
                else
                    dsp::move(pFftBuf, pInBuf, buf_size);

                // Shift input and output buffers
                dsp::move(pOutBuf, &pOutBuf[frame_size], buf_size + frame_size);
                dsp::fill_zero(&pOutBuf[frame_size], frame_size);

                // Apply window and add to the output buffer
                dsp::fmadd3(pOutBuf, pFftBuf, pWnd, buf_size);
