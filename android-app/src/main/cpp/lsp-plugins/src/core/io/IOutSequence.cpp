...
namespace lsp
{
    namespace io
    {
        ...
        status_t IOutSequence::write(const LSPString *s)
        {
            if (s == NULL)
                return set_error(STATUS_BAD_ARGUMENTS);
            return write(s->get_native(), s->length());
        }
        ...
        status_t IOutSequence::write(const LSPString *s, ssize_t first)
        {
            ...
            const lsp_wchar_t *v = s->get_native();
        ...
        {
            status_t res = write(s, first, last);
            if (res != STATUS_OK)
                return res;
            return write('\n');
        }

        status_t IOutSequence::flush()
        {
            return set_error(STATUS_NOT_IMPLEMENTED);
        }
