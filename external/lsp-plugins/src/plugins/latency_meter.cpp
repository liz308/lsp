...
namespace lsp
{
    ...
    void latency_meter::init(IWrapper *wrapper)
    {
        ...
        LOGI("Latency meter initialized");
    }
    ...
    void latency_meter::update_sample_rate(long sr)
    {
        ...
        sBypass.init(sr);
        LOGI("Sample rate updated to %ld", sr);
    }
    ...
    void latency_meter::process(size_t samples)
    {
        ...
        {
            float latency_ms = sLatencyDetector.get_latency_seconds() * 1000.0f;
            pLatencyScreen->setValue(latency_ms); // * 1000.0f to show ms instead of s
            LOGI("Latency detected: %.2f ms", latency_ms);
        }
    }
    ...
    void latency_meter::update_settings()
    {
        ...
        if (bTrigger)
        {
            ...
            LOGI("Latency capture triggered");
        }
...
extern "C" {
    ...
    Java_com_lsp_plugins_LatencyMeter_createNative(JNIEnv *env, jobject thiz) {
        lsp::latency_meter *meter = new lsp::latency_meter();
        return reinterpret_cast<jlong>(meter);
    }

    JNIEXPORT void JNICALL
    Java_com_lsp_plugins_LatencyMeter_destroyNative(JNIEnv *env, jobject thiz, jlong handle) {
        lsp::latency_meter *meter = reinterpret_cast<lsp::latency_meter*>(handle);
        if (meter) {
            meter->destroy();
            delete meter;
        }
    ...
    JNIEXPORT void JNICALL
    Java_com_lsp_plugins_LatencyMeter_processNative(JNIEnv *env, jobject thiz, jlong handle, jint samples) {
        lsp::latency_meter *meter = reinterpret_cast<lsp::latency_meter*>(handle);
        if (meter) {
            meter->process(static_cast<size_t>(samples));
        }
    }

    JNIEXPORT void JNICALL
    Java_com_lsp_plugins_LatencyMeter_updateSettingsNative(JNIEnv *env, jobject thiz, jlong handle) {
        lsp::latency_meter *meter = reinterpret_cast<lsp::latency_meter*>(handle);
        if (meter) {
    ...
    JNIEXPORT void JNICALL
    Java_com_lsp_plugins_LatencyMeter_updateSampleRateNative(JNIEnv *env, jobject thiz, jlong handle, jlong sampleRate) {
        lsp::latency_meter *meter = reinterpret_cast<lsp::latency_meter*>(handle);
        if (meter) {
            meter->update_sample_rate(static_cast<long>(sampleRate));
        }
    }
}
