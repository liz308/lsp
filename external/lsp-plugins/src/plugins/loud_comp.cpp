...
#define TRACE_PORT(p) __android_log_print(ANDROID_LOG_DEBUG, "LSP", "  port id=%s", (p)->metadata()->id);
...
namespace lsp
{
    ...
    void loud_comp_base::init(IWrapper *wrapper)
    {
        ...
        __android_log_print(ANDROID_LOG_DEBUG, "LSP", "Binding ports...");
        size_t port_id      = 0;
        ...
        if (c != NULL)
        {
            ...
            if (fft_size == 256)
            {
                for (size_t i=0; i<fft_size; ++i)
                    __android_log_print(ANDROID_LOG_DEBUG, "LSP", "i=%d; vfa={%.2f, %.2f}", int(i), vFreqApply[i*2], vFreqApply[i*2+1]);
            }
        }
        else
        {
